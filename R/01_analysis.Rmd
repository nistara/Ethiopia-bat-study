...
date_order = unique(format(as.Date(sort(df$EventDate), format = "%Y-%m-%d"),
                        "%Y-%B"))
df$EventDate = format(as.Date(df$EventDate, format = "%Y-%m-%d"), "%Y-%B")
df$EventDate = factor(df$EventDate, levels = date_order, ordered = TRUE)


p_df = df %>%
    group_by(ConcurrentSamplingSite, SeasonModelled, EventDate,
             Sex, AgeClass, is_positive) %>%
    arrange(EventDate) %>%
    summarize(n = n()) %>%
    mutate(is_positive = ifelse(is_positive, "Positive", "Negative")) %>%
    mutate(is_positive = factor(is_positive, levels = c("Positive", "Negative")))

levels(p_df$EventDate) = gsub("-", "\n", levels(p_df$EventDate))

event_p =

    ggplot(p_df, aes(x = EventDate, y = n, fill = is_positive, label = n)) +
...
cowplot::save_plot("results/figs/event-plot.pdf", event_p, base_height = 6)
cowplot::save_plot("results/figs/event-plot.png", event_p, base_height = 6)
...
lapply(sp_l, function(sp, out_dir) {
    ...
    if(nrow(sp) > 1) {
        ...
        out_n = file.path(out_dir, paste0(gsub(" ", "_", unique(sp$sp_e)), ".pdf"))
        cowplot::save_plot(out_n, sp_p, base_height = .5)
...
Note from Julius' paper: Fisher’s exact test was used to
